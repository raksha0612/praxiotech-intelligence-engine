...
# ─── PAGE CONFIG ──────────────────────────────────────────────────────────────────
st.set_page_config(
    page_title="Intelligence Engine v1.2",
    page_icon="⚡",
    layout="wide",
    initial_sidebar_state="expanded"
)

# ─── LIGHT-MODE CSS  (no dark theme conflicts) ────────────────────────────────────
st.markdown("""
<style>
...
[data-testid="stSidebar"] label { color: #94A3B8 !important; font-size:11px !important; text-transform:uppercase !important; letter-spacing:0.08em !important; }
...
.engine-badge {
  ...
  font-size:10px; font-weight:700; letter-spacing:0.08em; display:inline-block;
  margin-bottom:14px;
}
...
.action-row {
  ...
  background:#F8FAFC; margin-bottom:8px; cursor:pointer;
...
/* Remove default streamlit backgrounds that clash */
...
with st.sidebar:
    ...
    <div style="padding:18px 0 20px;border-bottom:1px solid #1E293B;margin-bottom:18px">
      ...
      <div style="font-size:11px;color:#475569">Revenue Audit Platform v1.2</div>
    </div>
    """, unsafe_allow_html=True)
    ...
    district = st.selectbox("", ["FRA-Standard", "FRA-Premium", "FRA-All"], label_visibility="collapsed", key="dist")

    st.markdown("---")
    st.markdown(f'<p style="color:#475569;font-size:11px;line-height:1.6">Analyzing <b style="color:#94A3B8">{len(restaurant_names)} establishments</b> across Frankfurt City. Data refreshed live.</p>', unsafe_allow_html=True)
    ...
    sw_list = df_rest[(df_rest['rating_n'] >= 4.5) & (df_rest['res_rate'].fillna(0) < 0.3)]['name'].head(4).tolist()
    for sw in sw_list:
        st.markdown(f'<div style="background:#1E293B;border-left:3px solid #EF4444;padding:7px 11px;border-radius:4px;margin-bottom:5px;font-size:11px;color:#FCA5A5">{sw}</div>', unsafe_allow_html=True)


# ─── COMPUTE (Section 1 to replace) ───────────────────────────────────────────────
scores      = compute_dimension_scores(selected_restaurant, df_rest, df_rev)
gaps        = get_gap_analysis(scores, benchmarks)
momentum    = compute_momentum(selected_restaurant, df_rev)
...
# ─── PAGE HEADER ──────────────────────────────────────────────────────────────────
silent_badge = '&nbsp;&nbsp;<span class="badge-silent">🔴 SILENT WINNER DETECTED</span>' if silent_flag else ''
st.markdown(f"""
<div class="page-header">
  <div class="engine-badge">⚡ Intelligence Engine Active v1.2</div>
  <h1 class="rest-name">{selected_restaurant}</h1>
  <div class="rest-meta">📍 District Context: Ranked <strong>#{cur_rank}</strong> of {total} establishments in Frankfurt City {silent_badge}</div>
</div>
""", unsafe_allow_html=True)
...
prev   = health - round(np.random.uniform(1.5, 3.5), 1)

tiles = [
    (c1, "Overall Score",    f"{health:.1f}",    "out of 100", f"↗ +{round(health-prev,1)}", True),
    (c2, "Rank",             f"#{cur_rank}",      "Frankfurt City", "↗ +2", True),
    (c3, "Responsiveness",   f"{scores['Responsiveness']:.0f}%", "Owner reply rate", "↗ +5%", True),
    (c4, "Sentiment",        f"{scores['Intelligence']:.0f}%", "Review sentiment",
     ("↗ +3%" if scores['Intelligence'] >= 75 else "↘ -2%"), scores['Intelligence'] >= 75),
    (c5, "Freshness",        f"{scores['Visibility']:.0f}%",  "Review velocity", "↗ +8%", True),
]
...
with col_radar:
    ...
    sv = [scores['Reputation'], scores['Responsiveness'], scores['Digital Presence'], scores['Intelligence'], scores['Visibility']]
    bv = [benchmarks.get('rating',4.4)*20, 90, 85, 75, 70]

    fig = go.Figure()
    fig.add_trace(go.Scatterpolar(r=sv+[sv[0]], theta=dim_labels+[dim_labels[0]],
        fill='toself', name='Score',
        line=dict(color='#0EA5E9', width=2.5),
        fillcolor='rgba(14,165,233,0.13)',
        marker=dict(size=5, color='#0EA5E9')))
    fig.add_trace(go.Scatterpolar(r=bv+[bv[0]], theta=dim_labels+[dim_labels[0]],
...
with col_act:
    ...
    actions = [
        ("⚡","Optimize Response Time",       "Reduce avg reply to under 2 hours — top revenue lever",
         f"Est. +{max(round((90-scores['Responsiveness'])*0.8),5):.0f} pts score lift", "HIGH",   "badge-high"),
...
with cm2:
    rev_url_col  = find_col(df_rev, ['page_url','url','link'])
    rest_url_val = str(res_data.get('Page_URL', res_data.get('page_url', '')))

    if rev_url_col and rest_url_val:
        sub = df_rev[df_rev[rev_url_col].astype(str) == rest_url_val]
    else:
        sub = pd.DataFrame()
...
<div style="text-align:center;padding:16px;color:#94A3B8;font-size:11px;
  ...
  ⚡ Intelligence Engine v1.2 &nbsp;·&nbsp; Praxiotech GmbH &nbsp;·&nbsp;
